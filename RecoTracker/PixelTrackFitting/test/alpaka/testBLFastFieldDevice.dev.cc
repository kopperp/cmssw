  ...
  void runLayout(Queue& queue, const float* map, const blMaterialMap::Map* rho, bool otOnly, const char* label) {
    ...
    for (int t = 0; t < nTracks; ++t)
      for (int i = 0; i < NH; ++i) {
        ...
        float_st* g = geHost.data() + std::size_t(t) * 6 * NH + 6 * i;
    ...
    *rho_h.data() = *rho;
    ...
    for (int t = 0; t < nNoiseless; ++t) {
      ...
      printf(
          ...
          " %8.2f %5.2f\n",
          static_cast<double>(kept[t].pT),
          static_cast<double>(kept[t].eta),
          kept[t].q,
          static_cast<double>(z0Off),
          static_cast<double>(z0On),
          static_cast<double>(z0Pca),
          static_cast<double>(z0Full),
          static_cast<double>(d0Pca),
          static_cast<double>(d0Full),
          static_cast<double>(cotOff),
          static_cast<double>(cotOn),
          static_cast<double>(at(t, 0)[4] / kept[t].pT),
          static_cast<double>(at(t, 1)[4] / kept[t].pT),
          static_cast<double>(at(t, 2)[4] / kept[t].pT),
          static_cast<double>(at(t, 0)[5]),
          static_cast<double>(at(t, 1)[5]));
      maxZOn = std::max(maxZOn, abs(z0On));
      maxZPca = std::max(maxZPca, abs(z0Pca));
      maxZFull = std::max(maxZFull, abs(z0Full));
      maxD0Pca = std::max(maxD0Pca, abs(d0Pca));
      maxD0Full = std::max(maxD0Full, abs(d0Full));
      maxCotOn = std::max(maxCotOn, abs(cotOn) * 1e-4);
      maxZOff = std::max(maxZOff, abs(z0Off));
      maxCotOff = std::max(maxCotOff, abs(cotOff) * 1e-4);
      maxPtOff = std::max(maxPtOff, abs(at(t, 0)[4] / kept[t].pT - 1.));
      maxPtOn = std::max(maxPtOn, abs(at(t, 1)[4] / kept[t].pT - 1.));
    ...
    printf(
        ...
        nNoiseless,
        static_cast<double>(maxZOff),
        static_cast<double>(maxZOn),
        static_cast<double>(maxZPca),
        static_cast<double>(maxZFull),
        static_cast<double>(maxD0Pca),
        static_cast<double>(maxD0Full),
        static_cast<double>(maxCotOff),
        static_cast<double>(maxCotOn),
        static_cast<double>(maxPtOff),
        static_cast<double>(maxPtOn),
        static_cast<double>(maxChi2On));
    ...
    CHECK(maxZFull < fmax(2.0, 0.2 * maxZOn));
    CHECK(maxD0Full < fmax(2.0, 0.1 * maxD0Pca));
    for (int t = 0; t < nNoiseless; ++t) {
    ...
    for (int t = 0; t < nNoiseless; ++t)
      CHECK(abs(at(t, 1)[4] - at(t, 2)[4]) < 0.01 * kept[t].pT);
    ...
    for (int t = 0; t < nNoiseless; ++t) {
      ...
      for (int u = 0; u < nNoiseless; ++u) {
        if (kept[u].pT == 1. && abs(kept[u].eta + kept[t].eta) < 1e-9 && kept[u].q == kept[t].q)
          tMirror = u;
        if (kept[u].pT == 1. && abs(kept[u].eta - kept[t].eta) < 1e-9 && kept[u].q == -kept[t].q)
          tCharge = u;
      }
    ...
    for (std::size_t c = 0; c < pullCfg.size(); ++c) {
      ...
      printf("                    %5.1f %+4.1f | %8.3f %9.3f | %8.3f %9.3f\n",
             ...
             static_cast<double>(sgz),
             static_cast<double>(mc),
             static_cast<double>(sgc));
      oddZ = std::max(oddZ, abs(mz));
      oddC = std::max(oddC, abs(mc));
      CHECK(abs(mz) < 0.3);
      CHECK(abs(mc) < 0.3);
      CHECK(sgz > 0.3);
      CHECK(sgz < 1.5);
    ...
    printf("   worst pull mean: z0 %.3f, cot %.3f\n", static_cast<double>(oddZ), static_cast<double>(oddC));
  }
